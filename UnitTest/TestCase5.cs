...
namespace UnitTest
{
	...
	public class TestCase5
	{
		...
		public void main()
		{
			...
			string url				= apiHost + "api/v1.0/FssLifeplan/RegisterStep1";


			// API 요청 파라미터 설정
			Dictionary<string, string> bodies	= new Dictionary<string, string>();
			bodies.Add("UserId"					, aesEncrypt(aesKey, aesIv, "userid"));
			bodies.Add("UserName"				, aesEncrypt(aesKey, aesIv, File.ReadAllText("D:\\username.txt")));
			bodies.Add("BirthDate"				, aesEncrypt(aesKey, aesIv, File.ReadAllText("D:\\birthdate.txt")));
			bodies.Add("IdentityNumber"			, aesEncrypt(aesKey, aesIv, File.ReadAllText("D:\\identitynumber.txt")));
			bodies.Add("MobileCorp"				, "1");
			bodies.Add("PhoneNumber"			, aesEncrypt(aesKey, aesIv, File.ReadAllText("D:\\phonenumber.txt")));
			bodies.Add("RegionCode1"			, "009");
			...
			using (HttpClient httpClient = new HttpClient())
			{
				...
				using (var response = httpClient.PostAsync(url, reqContent).Result)
				{
					...
					// 바이너리 파일 저장
					JObject resJson	= JObject.Parse(resContent);
					File.WriteAllBytes("D:\\captcha.png", Convert.FromBase64String((string)resJson["Result"]["CaptchaImage"]));
				}
			}
		}
	}
