	...
	public class 사대보험취득신고
	{
		...
		public void TILKO_API()
		{
			try
			{
				...
				_rest.AddBody("BusinessNumber"   , "", true);
				...
				_rest.AddBody("GeunrojaRgNo"     , "", true);   // [암호화] 근로자 주민등록번호 13자리
				_rest.AddBody("GeunrojaNm"       , "");         // 근로자 성명
				_rest.AddBody("JikJongCd"        , "");			// enum.txt 파일에서 [JikJongCd(직종부호코드)] 참고
				...
				_rest.AddBody("NPSChwideukBuho"  , "0");        // 국민연금 취득 부호 - 0: 십팔세이상당연취득, 1: 십팔세미만취득, 2: 전입사업장통폐합, 3: 대학강사, 4: 육십시간미만신청취득
				_rest.AddBody("NHICChwideukBuho" , "0");        // 건강보험 취득 부호 - 0: 최초취득, 1: 의료급여수급권자해제, 2: 직장가입자변경, 3: 직장피부양자상실, 4: 지역가입자에서변경, 5: 국가유공자상실, 6: 기타, 7: 직권말소후재등록, 8: 직장가입자이중가입
